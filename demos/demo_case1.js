...
function graphTemplate()
{
    ...
    node_tra.pos = [300,575];
    graph.add(node_tra);

    var node_grad = LiteGraph.createNode("Texture/Gradient");
    node_grad.pos = [540,575];
    graph.add(node_grad);

    var node_ramp = LiteGraph.createNode("Operator/ColorRamp");
    node_ramp.pos = [500,400];
    node_ramp.setMinValue(0.0);
    node_ramp.setMaxValue(1.0);
    graph.add(node_ramp);
    ...
    //Connections
    node_tf.connect(0, node_volume, 0);
    node_math.connect(0, node_volume, 1);
    node_ramp.connect(0, node_math, 1);
    node_grad.connect(1, node_ramp, 0);
    node_tra.connect(0, node_grad, 0);
    node_rot.connect(0, node_tra, 0);
